...
def handleMessage(msg):
    ...
    receiver = User.query.get(session['chat_session'][1])
    receiver.add_notification('unread_chats', receiver.unread_chats())
    db.session.add(msg)
    db.session.commit()
...
def readmsg():
    Chats.query.get(session['chat_session'][0]).last_msg().receiver_read = True
